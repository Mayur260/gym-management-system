...
const GYM_NAV = [
  {
    section: 'Main',
    items: [
      { label: 'Dashboard',   page: 'dashboard.html',   icon: iconGrid(),     badge: '' },
      { label: 'Members',     page: 'members.html',     icon: iconUsers(),    badge: '128' },
      { label: 'Trainers',    page: 'trainers.html',    icon: iconDumbbell(), badge: '' },
      { label: 'Memberships', page: 'memberships.html', icon: iconCard(),     badge: '' },
    ]
  },
  {
    section: 'Operations',
    items: [
      { label: 'Attendance', page: 'attendance.html', icon: iconCalendar(), badge: '' },
      { label: 'Payments',   page: 'payments.html',   icon: iconCash(),     badge: '5' },
      { label: 'Reports',    page: '#',               icon: iconChart(),    badge: '' },
    ]
  },
  {
    section: 'Account',
    items: [
      { label: 'Settings', page: 'settings.html', icon: iconSettings(), badge: '' },
    ]
  }
];
...
function buildSidebar(pageTitle) {
  ...
  const navHtml = GYM_NAV.map(group => `
    ...
    ${group.items.map(item => `
      <a href="${item.page}" class="nav-item ${item.page === page ? 'active' : ''}" data-page="${item.page}">
        ${item.icon}
  ...
  const sidebar = `
    <aside class="sidebar" id="sidebar">
      ...
      <div class="sidebar-footer">
        <a href="settings.html" class="sidebar-user">
        ...
        <a href="index.html" class="nav-item mt-2" style="color:var(--danger);margin-top:6px;">
          ${iconLogout()}
          <span>Logout</span>
        </a>
      </div>
  ...
  const topbar = `
    <header class="topbar">
      ...
      <div class="topbar-search">
        <div class="topbar-search-icon"><svg viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z" clip-rule="evenodd"/></svg></div>
      ...
      <div class="topbar-actions">
        ...
        <a href="settings.html" class="topbar-avatar" title="Profile">AR</a>
      </div>
    </header>
  `;
